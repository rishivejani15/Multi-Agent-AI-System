...
def run_pipeline(state: dict, max_iterations: int = 3) -> dict:
    ...
    for iteration in range(max_iterations):
        ...
        # ← here we call run(goal, state) instead of .process()
        state = agents["PlannerAgent"].run(state.get("goal", ""), state)
        ...
        for agent_name in state["plan"]:
            if agent_name in agents:
                # most of your agents ignore the first arg, so we pass ""
                state = agents[agent_name].run("", state)
            else:
...
def main():
    ...
    if launch_data:
        print("\n🚀 Launch Info:", launch_data.get("mission_name"),
              "-", launch_data.get("launch_date_utc"))

    weather_data = state.get("weather_data", {})
    if weather_data and "weather" in weather_data and "main" in weather_data:
        print("\n🌦️ Weather:", weather_data["weather"][0]["main"],
